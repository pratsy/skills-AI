...
- `skills_ai/` = the Python SDK that reads those same `SKILL.md` files, sends them to a model provider along with your input data, and evaluates the output's structure.
...
`skills_ai.runner.run_skill(skill_name, input_path)`:

1. Looks for a bespoke `skills_ai/skills/<skill_name>.py` module (an escape hatch for a skill that needs custom input handling — none currently exist; all 45 skills run generically).
